...
## Карта миграции существующей логики

| Тип логики в текущей системе                        | Куда переезжает                                                          | Что меняется                                                                                                                                           | Переходный механизм                                                                                                        |
|-----------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| Маршрутизация сообщений между системами             | `Event API`, `Batch Ingestion API`, `Validation & Routing`, Kafka topics | Вместо центральных Camel routes используются доменные события, топики и подписчики. Маршруты становятся частью контрактов событий и правил публикации. | `Legacy Adapter` читает старые маршруты Camel и публикует нормализованные события через `Event API`, `Batch Ingestion API` |
| Трансформации форматов и контрактов                 | `Legacy Event Mapper`, `Schema Registry`, `Stream Transformations`       | Технические преобразования выполняются на входе и в stream processing, а аналитические преобразования фиксируются в доменных data products.            | Старые форматы DWH/Camel преобразуются в доменные события внутри `Legacy Event Mapper`.                                    |
| Бизнес-логика                                       | `Domain Data Products`                                                   | Бизнес-правила уходят из ESB и DWH к владельцам доменов.                                                                                               | Новый продукты заводятся в Domain Products, старые постепенно переносятся. На время миграции - синхронизация через ACL     |
| Интеграционная логика с партнерами (события)        | `Events API`                                                             | Интеграции описываются через явные контракты вместо скрытых точечных маршрутов в Camel.                                                                | Legacy-интеграции постепенно выносятся из Camel в `Events API` и отключаются по доменам.                                   |
| Интеграционная логика с партнерами (пакеты и файлы) | `Batch Ingestion API`                                                    | Пакетные сценарии остаются поддержанными, но оформляются как отдельный входной поток с валидацией и публикацией в Kafka.                               | Legacy-интеграции постепенно выносятся из Camel в `Batch Ingestion API` и отключаются по доменам.                          |
